namespace Mercury.ParticleEngine.Renderers {
    ...
    public class SlimDXPointSpriteRenderer : IDisposable
    {
        ...
        public SlimDXPointSpriteRenderer(Device device, int size, IReadOnlyDictionary<String, Texture> textureLookup)
        {
            if (device == null)
            ...
            _effect = Effect.FromString(device, SlimDXResources.PointSpriteShader, ShaderFlags.PartialPrecision);
        ...
        internal void Render(Emitter emitter, Matrix worldViewProjection)
        {
            ...
            var vertexDataPointer = _vertexBuffer.Lock(0, emitter.Buffer.ActiveSizeInBytes, LockFlags.Discard);

            switch (emitter.RenderingOrder)
            {
                case RenderingOrder.FrontToBack:
                    {
                        emitter.Buffer.CopyTo(vertexDataPointer.DataPointer);
                        break;
                    }
                case RenderingOrder.BackToFront:
                    {
                        emitter.Buffer.CopyToReverse(vertexDataPointer.DataPointer);
                        break;
                    }
            }
